...
- **Source Citations**: Automatic extraction and display of source files with quotes
- **Citation Cleaning**: Removal of citation markers (e.g., `【4:0†filename.md】`) from response text
...
3. ✅ **Structured Output**: Enhanced response format with citations
4. ✅ **Metrics Dashboard**: Comprehensive monitoring and analytics
...
- "Here's kpi_tracker.csv — visualize our Q1 progress vs targets"
...
│   ├── router.py            # Multi-assistant routing logic
...
│   ├── investor/             # Fundraising resources
...
├── scripts/
│   ├── seed_knowledge.py         # Initialize single assistant (legacy)
│   ├── seed_multi_assistants.py  # Initialize multi-assistant system
│   └── update_assistant.py      # Update existing assistant instructions
├── .env.example
├── requirements.txt
├── Dockerfile
├── docker-compose.yml
├── MULTI_ASSISTANT_ROUTING.md   # Multi-assistant routing documentation
└── EXAMPLE_QUESTIONS.md         # Example questions for testing
```
...
1. Add files to the appropriate directory:
   - Technical docs → `data/tech/`
   - Marketing resources → `data/marketing/`
   - Fundraising/financial → `data/investor/`
...
1. **Knowledge Base Setup**: Files are organized by domain:
   - `data/tech/` → TechAdvisor vector store
   - `data/marketing/` → MarketingAdvisor vector store
   - `data/investor/` → InvestorAdvisor vector store
